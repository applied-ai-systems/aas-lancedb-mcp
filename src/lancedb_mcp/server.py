...
async def handle_call_tool(
    ...
    try:
        ...
        if name == "create_table":
            ...
            logger.info(
                f"Created table {config.name} with dimension {config.dimension}"
            )
            return [
                types.TextContent(
                    type="text",
            ...
            try:
                ...
                metadata_rows = (
                    table.search([0.0] * 384)
                    .where("source = '__SYSTEM__'")
                    .limit(1)
                    .to_pandas()
                )
                if not metadata_rows.empty and metadata_rows.iloc[0]["text"].startswith(
                    "__METADATA__"
                ):
                    metadata_json = metadata_rows.iloc[0]["text"].replace(
                        "__METADATA__", ""
                    )
                ...
                logger.warning(
                    f"Could not retrieve table metadata, using default model: {e}"
                )
            ...
            for result in results_dict:
                if "_distance" in result:
                    result["similarity_score"] = 1.0 - float(
                        result["_distance"]
                ...
                if "vector" in result:
                    ...
                    if (
                        len(results_dict) <= 5
                    ):  # Only include vectors for small result sets
                        result["vector"] = result["vector"].tolist()
                    else:
                        del result["vector"]  # Remove vector for large result sets

            logger.info(
                f"Semantic search in table {table_name} returned {len(results_dict)} results"
            )
            return [
                types.TextContent(
                    type="text",
            ...
            logger.info(
                f"Vector search in table {table_name} returned {len(results_dict)} results"
            )
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"results": results_dict, "total_results": len(results_dict)},
                        indent=2,
                    ),
                )
            ]
            ...
            for table_name in table_names:
                try:
                    ...
                    tables_info.append(
                        {"name": table_name, "row_count": "Error", "error": str(e)}
                    )
            ...
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"tables": tables_info, "total_tables": len(table_names)},
                        indent=2,
                    ),
                )
            ]
            ...
            return [types.TextContent(type="text", text=info.model_dump_json(indent=2))]
            ...
            return [
                types.TextContent(type="text", text=f"Deleted table '{table_name}'")
            ]
