...
class ColumnSchema(BaseModel):
    ...
    type: Literal["text", "integer", "float", "boolean", "json"] = Field(
        ..., description="Column data type"
    )
    searchable: bool = Field(
        default=False,
        description="Whether this text column should be searchable via embeddings",
    )
...
class TableSchema(BaseModel):
    ...
    embedding_model: str = Field(
        default="BAAI/bge-m3", description="Embedding model for searchable text columns"
    )
...
class QueryData(BaseModel):
    ...
    columns: list[str] | None = Field(
        default=None, description="Columns to return (null for all)"
    )
    where: dict[str, Any] | None = Field(
        default=None, description="Conditions to match"
    )
...
class SearchQuery(BaseModel):
    ...
    where: dict[str, Any] | None = Field(
        default=None, description="Additional filtering conditions"
    )
...
class TableInfo(BaseModel):
    ...
    searchable_columns: list[str] = Field(
        ..., description="Columns with embedding search enabled"
    )
...
class MigrationPlan(BaseModel):
    ...
    column_mappings: dict[str, str] | None = Field(
        default=None, description="Mapping from old to new column names"
    )
    data_transformations: dict[str, str] | None = Field(
        default=None, description="SQL-like expressions for data transformation"
    )
    backup_name: str | None = Field(
        default=None,
        description="Name for backup table (auto-generated if not provided)",
    )
...
def create_table_model(schema: TableSchema, dimension: int = 1024):
    ...
    try:
        ...
        for col in schema.columns:
            ...
            if col.type == "text":
                ...
                fields[col.name] = Field(
                    default=field_default, description=col.description
                )
                ...
                if col.searchable:
                    ...
                    fields[vector_name] = Field(
                        default=None,
                        dim=dimension,
                        description=f"Embedding for {col.name}",
                    )
                ...
                if col.required:
                    ...
                    fields[col.name] = Field(
                        default_factory=dict, description=col.description
                    )
        ...
        fields["table_metadata"] = Field(
            default=None, description="JSON table metadata"
        )
...
def get_embedding_columns(schema: TableSchema) -> dict[str, str]:
    ...
    return {
        col.name: f"{col.name}_vector"
        for col in schema.columns
        if col.type == "text" and col.searchable
    }
